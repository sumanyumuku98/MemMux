...
\sys{} 0.8.0    & \cmark & \textbf{100\%}& \cmark~10/10 & \textbf{435\,MiB} & \cmark & 2.7\% \\
\bottomrule
\end{tabular}
...
footprint under overcommit (H4), and checkpoints dirty work before any reclaim. It \emph{matches}
\code{tmux} on teardown cleanup (H2) and---stated plainly---pays a monitoring cost the ungoverned
tools do not (H5). Each cell is the headline result for that signal; the subsections give the detail.}
\label{tab:matrix}
\end{table}
...
not a tuning difference, and it is the cleanest separation from a real competitor.
...
owning task (H1). The cost of governance in \emph{memory} is small and we report it plainly: the \sys{}
