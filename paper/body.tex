...
\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=2.6mm and 15mm]
  % --- client ---
  ...
  \node[font=\scriptsize\bfseries\color{syscol}, anchor=south] at (daemon.north)
        {\code{memmuxd} daemon};
  % --- owned process trees ---
  \node[root, right=of attr] (rA) {agent A};
  \node[proc, below=6.5mm of rA, xshift=-10mm] (a1) {LSP};
  \node[proc, below=6.5mm of rA, xshift=10mm]  (a2) {tests};
  \node[root, below=20mm of rA] (rB) {agent B};
  \node[proc, below=6.5mm of rB, xshift=-10mm] (b1) {shell};
  \node[proc, below=6.5mm of rB, xshift=10mm]  (b2) {build};
  \begin{scope}[on background layer]
  ...
  \node[font=\scriptsize\bfseries\color{inkcol}, anchor=south] at (trees.north)
        {owned process trees};
  % --- escaped child ---
  \node[escaped, below=9mm of b2] (esc) {escaped child\\reparented to \code{init}};
  % --- tree edges ---
  \draw[flow] (rA) -- (a1); \draw[flow] (rA) -- (a2);
  \draw[flow] (rB) -- (b1); \draw[flow] (rB) -- (b2);
  \draw[esc]  (b2) -- (esc);
  % --- control + measurement edges ---
  \draw[biflow] (client.east) -- node[above,font=\scriptsize]{UDS} (daemon.west|-client);
  \draw[flow] (daemon.east|-rA) -- node[above,font=\scriptsize,align=center]{launch\\\& own} (rA.west);
  \draw[scan] (attr.east) to[out=0,in=180]
        node[above,font=\scriptsize,pos=0.55]{$1$\,Hz PSS scan} (trees.west|-attr);
  \draw[esc] (esc.south) -- ++(0,-4mm) -|
...
\begin{axis}[memmux,
  ...
  xtick={1,5,10}, ymin=0, ymax=2450, legend pos=north west,
]
\addplot[rawcol,  mark=*]         coordinates {(1,215.3)(5,1073.7)(10,2146.2)}; \addlegendentry{raw baseline}
\addplot[tmuxcol, mark=square*]   coordinates {(1,217.7)(5,1076.1)(10,2148.6)}; \addlegendentry{\code{tmux} 3.4}
\addplot[syscol,  mark=triangle*, mark size=3pt] coordinates {(1,219.9)(5,434.8)(10,434.8)}; \addlegendentry{\sys{} 0.8.0}
% annotate the separation at N=10
\draw[{Stealth[length=5pt]}-{Stealth[length=5pt]}, accentcol, line width=0.9pt]
  (axis cs:10,434.8) -- (axis cs:10,2146.2);
\node[fill=white, inner sep=1.5pt, font=\footnotesize\bfseries, color=accentcol]
  at (axis cs:10,1300) {$4.9\times$};
\end{axis}
\end{tikzpicture}
...
task). Ten seconds later we count survivors from the owned set (Figure~\ref{fig:h2}). The raw baseline
...
\begin{axis}[memmux,
  ...
  ybar, bar width=8pt, enlarge x limits=0.28,
  symbolic x coords={1,5,10}, xtick=data,
  xlabel={Fleet size $N$ (agents)}, ylabel={Leaked memory (MiB)},
  ymin=0, ymax=340, legend pos=north west,
  nodes near coords, every node near coord/.append style={font=\tiny, color=inkcol},
]
\addplot[fill=rawcol!85,  draw=rawcol]  coordinates {(1,30.6)(5,151.8)(10,303.0)}; \addlegendentry{raw baseline}
\addplot[fill=tmuxcol!85, draw=tmuxcol] coordinates {(1,0)(5,0)(10,0)};            \addlegendentry{\code{tmux}}
\addplot[fill=syscol!85,  draw=syscol]  coordinates {(1,0)(5,0)(10,0)};            \addlegendentry{\sys{}}
\end{axis}
\end{tikzpicture}
\caption{Memory stranded ten seconds after normal teardown (\emph{hold}). The raw baseline kills only
the roots, so it leaks a grandchild per agent---half of every fleet, growing to 303\,MiB at $N{=}10$
(50\% reclaimed). \code{tmux} and \sys{} reap their full subtree at every size (0 leaked, 100\%
reclaimed).}
\label{fig:h2}
\end{figure}
...
and 2.7\% at ten---over our 2\% target once the fleet reaches ten (Figure~\ref{fig:h5}). The scan is
$O(\text{processes})$ and currently re-reads the whole tree each tick; incremental sampling that
touches only changed pids is the obvious fix and is future work. We state the miss rather than bury
it.
...
\begin{tikzpicture}
\begin{axis}[memmux,
  width=0.82\linewidth, height=4.8cm,
  xlabel={Fleet size $N$ (agents)}, ylabel={Monitor CPU (\%)},
  xtick={1,5,10}, ymin=0, ymax=3.1, legend pos=north west,
]
\addplot[rawcol, dashed, line width=1pt, mark=none] coordinates {(1,2)(10,2)}; \addlegendentry{2\% target}
\addplot[syscol, mark=*] coordinates {(1,0.6)(5,1.5)(10,2.7)}; \addlegendentry{\sys{} monitor}
\node[font=\tiny, color=inkcol, anchor=south west] at (axis cs:1,0.6) {0.6};
\node[font=\tiny, color=inkcol, anchor=south]      at (axis cs:5,1.5) {1.5};
\node[font=\tiny, color=inkcol, anchor=south east] at (axis cs:10,2.7) {2.7};
\end{axis}
\end{tikzpicture}
\caption{Cost of the always-on $1$\,Hz attribution scan (H5), the paper's honest weak spot. Overhead
is linear in the process count and crosses the 2\% target between five and ten agents; incremental
sampling is the planned fix. The baselines run no such monitor.}
\label{fig:h5}
\end{figure}
...
on real trees}, not the overcommit footprint differentiator of Figure~\ref{fig:vsn}.}
\label{tab:realagent}
\begin{tabular}{lrrr}
\toprule
